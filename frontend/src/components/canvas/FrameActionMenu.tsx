...
export const FrameActionMenu = ({ shapeId }: { shapeId: TLShapeId }) => {
  ...
  useEffect(() => {
    if (
      initialPromptTextRef.current === null &&
      frame?.meta?.promptText &&
      typeof frame.meta.promptText === "string"
    ) {
  ...
  const handleImprove = async (e: React.MouseEvent) => {
    e.stopPropagation();
    console.log("[Improve Frame] Starting...");

    const currentFrame = editor.getShape(shapeId);
    if (!currentFrame) {
      console.log("[Improve Frame] No frame found, aborting");
      return;
    }

    setIsImproving(true);
    console.log("[Improve Frame] Set isImproving=true");
    ...
    try {
      ...
      console.log("[Improve Frame] Exporting frame as image...");
      ...
      console.log("[Improve Frame] Image exported, blob size:", blob?.size);
      ...
      console.log("[Improve Frame] Response status:", response.status);

      if (!response.ok) {
        const errorData = await response
          .json()
          .catch(() => ({ error: "Unknown error" }));
        console.log("[Improve Frame] Error response:", errorData);
        throw new Error(errorData.error || "Failed to improve image");
      }

      const result = await response.json();
      console.log("[Improve Frame] Got result, has image_bytes:", !!result.image_bytes);
      ...
      console.log("[Improve Frame] Processing base64 image data...");
      const imageDataUrl = processBase64ImageData(result.image_bytes);
      console.log("[Improve Frame] Validating image load...");
      const img = await validateImageLoad(imageDataUrl);
      console.log("[Improve Frame] Image validated, dimensions:", img.width, "x", img.height);

      // Create asset from improved image
      console.log("[Improve Frame] Creating asset...");
      const assetId = AssetRecordType.createId();
      const asset: TLImageAsset = {
        id: assetId,
        type: "image",
      ...
      console.log("[Improve Frame] Deleting", childIds.length, "existing children");
      ...
      console.log("[Improve Frame] Creating new image shape...");
      editor.createAssets([asset]);
      editor.createShapes([imageShape]);

      console.log("[Improve Frame] SUCCESS - Done!");
      ...
      console.error("[Improve Frame] ERROR:", error);
      ...
      toast.error(`Failed to improve frame: ${errorMessage}`);
      setIsImproving(false);
